...
This doesn't change anything in KrakenKey's flow today. We validate with `dns-01` and `http-01`, and `dns-persist-01` is not yet in any production CA we issue against, so there is no record format for us to migrate. It matters if you run your own ACME client and have been prototyping against Let's Encrypt staging or Pebble: the records you have already published are now the wrong shape, the fix requires code rather than a search and replace, and a draft that has broken its own record format once between revisions is not one to bake into zone templates yet.
